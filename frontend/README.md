...
默认开发地址为 `http://localhost:5173`，`/api` 请求会代理到 `VITE_PROXY_TARGET`（默认 `http://localhost:8200`）。
...
`Dockerfile` 使用 Node 22 构建静态资源，并通过 Nginx 80 端口提供服务。Nginx 将 `/api/` 反向代理到 `backend:8200`，并为 Vue Router 配置了 SPA fallback。

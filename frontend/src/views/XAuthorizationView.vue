...
<template>
  <section class="source-page" v-loading="loading">
    ...
    <div class="control-bar">
      <div><strong>{{ isActive ? '该数据源正在处理所有监听任务' : '配置完成后启用该数据源' }}</strong><span>{{ isActive ? 'Worker 会在每次读取前确认当前选择' : '切换会清除旧认证闸门，并把活跃任务重新排队' }}</span></div>
...
<style scoped>
.source-page{display:flex;flex-direction:column;gap:16px;max-width:1180px}.hero,.panel,.provider-card,.control-bar{border:1px solid var(--border-color);background:var(--surface-primary);border-radius:14px}.hero{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:16px;padding:22px;background:linear-gradient(120deg,rgba(104,79,255,.18),rgba(16,19,28,.96) 42%)}.hero__icon{display:grid;place-items:center;width:52px;height:52px;color:#b9adff;border:1px solid rgba(139,118,255,.35);border-radius:13px;background:rgba(105,79,255,.2)}.eyebrow{font-size:10px;letter-spacing:.16em;color:#9888ff}.hero h2{margin:4px 0 5px;font-size:23px}.hero p,.provider-card small,.control-bar span,.guide li,.cookie-steps,.risk-banner p,.security-copy{color:var(--text-secondary);font-size:12px;line-height:1.65}.active-source{min-width:190px;padding:12px 14px;border:1px solid rgba(89,213,163,.2);border-radius:10px;background:rgba(10,14,20,.48)}.active-source small,.active-source strong{display:block}.active-source strong{margin:3px 0 6px}.active-source span{display:flex;align-items:center;gap:6px;color:#68d7aa;font-size:9px;letter-spacing:.08em}.active-source i{width:6px;height:6px;border-radius:50%;background:#4fe3a7}.provider-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.provider-card{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:13px;padding:17px;text-align:left;color:inherit;cursor:pointer;transition:.2s}.provider-card:hover,.provider-card.is-selected{border-color:#7565dd;transform:translateY(-1px)}.provider-card__icon{display:grid;place-items:center;width:42px;height:42px;border-radius:11px;background:rgba(117,101,221,.13);color:#9d8fff}.provider-card__body{display:flex;flex-direction:column;gap:5px}.provider-card strong{display:flex;align-items:center;gap:8px}.provider-card em{padding:2px 6px;border-radius:10px;background:rgba(117,101,221,.15);color:#aa9dff;font-size:9px;font-style:normal}.active-pill{display:flex;align-items:center;gap:4px;color:#56d7a1;font-size:11px}.control-bar{display:flex;align-items:center;justify-content:space-between;padding:13px 16px}.control-bar>div{display:flex;flex-direction:column;gap:3px}.control-bar strong{font-size:13px}.activate-button,.primary-button{display:flex;align-items:center;justify-content:center;gap:7px;border:0;border-radius:8px;background:linear-gradient(120deg,#6d59dd,#806cf2);color:#fff;font-weight:650;cursor:pointer}.activate-button{padding:10px 15px}.activate-button:disabled,.primary-button:disabled{opacity:.45;cursor:not-allowed}.workspace-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.panel{padding:19px}.panel__head{display:flex;align-items:center;justify-content:space-between;margin-bottom:17px}.panel__head>div{display:flex;align-items:center;gap:9px}.panel h3{margin:0;font-size:15px}.step{display:grid;place-items:center;width:27px;height:27px;border-radius:8px;background:rgba(116,96,225,.17);color:#a697ff;font-size:10px}.step.warning{background:rgba(239,173,70,.14);color:#f0b45c}.method-tabs{display:grid;grid-template-columns:1fr 1fr;padding:4px;border-radius:9px;background:var(--surface-secondary)}.method-tabs button{padding:9px;border:0;border-radius:7px;background:transparent;color:var(--text-secondary);cursor:pointer}.method-tabs button.active{background:#6d59dd;color:white}.guide{margin-top:14px;padding:15px;border:1px solid var(--border-color);border-radius:10px;background:var(--surface-secondary)}.guide ol{display:grid;grid-template-columns:1fr 1fr;gap:7px 20px;padding-left:18px}.guide a,.terms-link{display:inline-flex;align-items:center;gap:4px;color:#9c8fff;font-size:11px;text-decoration:none}.credential-panel{display:flex;flex-direction:column;gap:12px}.status-strip{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.status-strip span{display:grid;grid-template-columns:auto 1fr;gap:1px 6px;padding:9px;border:1px solid var(--border-color);border-radius:9px;background:var(--surface-secondary)}.status-strip svg{grid-row:1/3;color:#8f80ed}.status-strip small{color:var(--text-secondary);font-size:9px}.status-strip strong{overflow:hidden;text-overflow:ellipsis;font-size:11px}.primary-button{width:100%;padding:11px}.primary-button.experimental{background:linear-gradient(120deg,#a66524,#d09240)}.manage-actions{display:flex;align-items:center;gap:8px;padding-top:10px;border-top:1px solid var(--border-color)}.manage-actions button{display:flex;align-items:center;gap:5px;padding:8px 10px;border:1px solid var(--border-color);border-radius:7px;background:transparent;color:inherit;cursor:pointer}.manage-actions button.danger{color:#ff7979}.manage-actions span{margin-left:auto;color:var(--text-secondary);font-size:10px}.last-error{margin:0;padding:9px;border-radius:7px;background:rgba(230,76,76,.09);color:#ff8989;font-size:11px}.risk-banner{display:flex;gap:10px;padding:12px;border:1px solid rgba(231,164,65,.25);border-radius:10px;background:rgba(231,164,65,.08);color:#e9ae55}.risk-banner p{margin:4px 0 0}.cookie-steps{padding-left:20px}.cookie-steps code{color:#c5baff}.security-copy{display:flex;gap:7px;margin:0}.spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:900px){.hero{grid-template-columns:auto 1fr}.active-source{grid-column:1/-1}.provider-grid,.workspace-grid{grid-template-columns:1fr}}@media(max-width:560px){.hero{padding:17px}.provider-grid{grid-template-columns:1fr}.control-bar{align-items:flex-start;gap:12px;flex-direction:column}.status-strip{grid-template-columns:1fr}.guide ol{grid-template-columns:1fr}}
.hero,.panel,.provider-card,.control-bar{border-color:var(--border);border-radius:16px;background:#fff;box-shadow:0 1px 2px rgba(18,24,40,.025),0 8px 28px rgba(18,24,40,.035)}
.hero{background:#fff}.hero__icon{color:var(--primary);border-color:#dcd9ff;background:var(--primary-soft)}.eyebrow{color:var(--primary);letter-spacing:0}.active-source{background:#fafbfc}.provider-card__icon{color:var(--primary);background:var(--primary-soft)}
.hero p,.provider-card small,.control-bar span,.guide li,.cookie-steps,.risk-banner p,.security-copy{color:var(--muted)}.activate-button,.primary-button{border-radius:10px;background:var(--primary)}.primary-button.experimental{background:var(--orange)}.method-tabs,.guide,.status-strip span{background:var(--surface-2)}.guide,.status-strip span,.manage-actions{border-color:var(--border)}.method-tabs button.active{background:#fff;color:var(--text);box-shadow:0 1px 4px rgba(18,24,40,.1)}.last-error{color:#b4233c;background:#fff5f6}.cookie-steps code{color:var(--primary)}
</style>

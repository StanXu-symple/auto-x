...
X Sentinel 是一个面向单团队/单管理员的 X（Twitter）公开账号监控与 AI 草稿系统。管理员可以在运行时新增、暂停、恢复或删除监听账号，并为每个账号单独调整轮询周期。系统将拉取到的公开 Post 持久化到 MySQL，使用 Redis 完成分布式互斥、Worker 心跳和短期缓存，并可通过独立 AI Worker 调用 OpenAI 或 Codex Bridge 生成待人工审阅的草稿。

系统只调用 X 官方 API，不包含网页抓取或绕过平台限制的逻辑。
...
1. 管理员启用 AI，并选择 provider、模型、默认 Skill、重试次数和输出限制；AI 默认关闭。
2. 新 Post 入库后可按配置自动创建任务，也可由管理员对指定 Post 手动创建。幂等键避免同一自动任务重复入队。
3. AI Worker 分批领取到期任务，使用数据库 claim token 与 Redis 租约避免多个实例重复提交。
4. Worker 把管理员维护的 Skill 作为可信编辑指令，把原 Post 放入明确标记的不可信 SOURCE 数据区，再调用 OpenAI 或 Codex Bridge。
5. 返回内容必须通过结构化输出校验；成功结果写入可编辑草稿，失败任务按配置退避重试，达到上限后保留错误状态供审计。
6. 草稿始终需要人工核验和后续操作，系统不包含自动发布到 X 的步骤。
...
`ai_settings` 保存是否启用、provider、模型、默认 Skill 和任务限制；`ai_skills` 保存可版本化的编辑说明与可选结构化输出约束。provider 密钥不进入这些表。
...
- 管理接口需要 JWT；初始管理员凭据、JWT 密钥、X Token 与 AI provider 密钥全部来自环境变量。
- `OPENAI_API_KEY` / `CODEX_BRIDGE_TOKEN` 只注入 AI Worker，不写入数据库、请求快照或日志；切勿在管理台 Skill、Post 或日志中粘贴密钥。
